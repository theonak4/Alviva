...
if($page == "" || $page == null || $page < 0) {
  header("Location: http://54.188.51.218/managerpanel/requests/?page=0");
} else if($page == 0) {
...
if($_SESSION["loggedIn"] == false) {
  ...
  $getAccepted = $conn->query("SELECT * FROM accessrecords WHERE playerId={$playerId} AND authorized=0");
...
<body>
  ...
  <br>
  <a href="http://54.188.51.218/managerpanel/" style="color: black"><h5><img src="managericon.png" alt="" width="36" height="36" style="margin-right: 10px; margin-left: 3%;"> your team</h5></a>
  <br>
  ...
  <br>
  <h5><img src="icons/box-arrow-in-right.svg" alt="" width="36" height="36" style="margin-right: 10px; margin-left: 3%;"> <strong>access requests</strong> <?php if($getAR->num_rows > 0) { echo "<span class='badge badge-primary badge-pill'>{$arNotifs}</span>"; } ?></h5>
  <br>
  <a href="http://54.188.51.218/managerpanel/history" style="color: black"><h5><img src="icons/book-half.svg" alt="" width="36" height="36" style="margin-right: 10px; margin-left: 3%;"> access history</h5>
  <br>
  ...
  <div style="position: fixed; width: 50%; height: 100%; top: 5%; left: 425px;">
    <h1 style="margin-top: 5%; margin-bottom: 20">Access Requests</h1>
    <?php
    $accessRecords = $conn->query("SELECT * FROM accessrecords WHERE playerId='{$playerId}' AND authorized=0 ORDER BY dateAuthorized DESC LIMIT 2 OFFSET {$aheadIndex}");
    if($accessRecords->num_rows > 0) {
      while($row = $accessRecords->fetch_assoc()) {
          ?>

        <span href="#" class="list-group-item" style="border: 0; background: #fafafa">
          ...
          <ul class="list-group list-group-horizontal-lg">
            <li class="list-group-item" style="border: 0; padding: 0; background: #fafafa"><small style="font-weight: 600">date requested <h4 style="font-weight: 500; width: 250"><strong><?php echo date("F jS, Y", strtotime($row["date"])) ?></strong></h4></small></li>
            <li class="list-group-item" style="border: 0; padding: 0; margin-left: 30; background: #fafafa"><small style="font-weight: 600">reason <h4 style="font-weight: 500"><?php echo $row["reason"]; ?></h4></small></li>
          </ul>
          <br>
          <small style="font-weight: 600">information requested</small>

          <ul class="list-group list-group-horizontal" style="margin-top: 5;">
            <li class="list-group-item <?php if($row["d1"]) { echo 'list-group-item-primary'; } else { echo 'list-group-item-light'; } ?>" style="border-radius: 0;">Birth Certificates</li>
            <li class="list-group-item <?php if($row["d2"]) { echo 'list-group-item-primary'; } else { echo 'list-group-item-light'; } ?>" style="border-radius: 0;">CalSouth Registration Forms</li>
            <li class="list-group-item <?php if($row["d3"]) { echo 'list-group-item-primary'; } else { echo 'list-group-item-light'; } ?>" style="border-radius: 0;">Player Cards</li>
            <li class="list-group-item <?php if($row["d4"]) { echo 'list-group-item-primary'; } else { echo 'list-group-item-light'; } ?>" style="border-radius: 0;">Insurance Waiver</li>
            <li class="list-group-item <?php if($row["d5"]) { echo 'list-group-item-primary'; } else { echo 'list-group-item-light'; } ?>" style="border-radius: 0;">COVID-19 Waiver</li>
          </ul>
          <br>
          <form action="http://54.188.51.218/managerpanel/release.php" method="POST">
          <input type="hidden" name="recordID" value="<?php echo $row['recordId']; ?>" />
          <button type="submit" class="btn btn-success" style="color: white; font-weight: 600; width: 300; border-radius:0;">authorize request</button>
          <button type="button" class="btn btn-danger" style="font-weight: 600; color: white; border-radius: 0; width: 200">deny request</button>
          </form>
    ...
    ?>
    <br>
    <div class="card" style="width: 600; border:0; padding: 0; margin: 0; text-align: center">
      <div class="card-body" style="padding: 0;">
        <img src="icons/inbox.svg" width="50" height="50"></img><br>
        <strong>Nothing to see here.</strong><br>
        Your team's information has not been requested yet.
        <br>
        <br>
  ...
  ?>
  <div class="btn-group">
  <a href="http://54.188.51.218/panel/requests/?page=<?php echo $page-1; ?>" ><button type="button" class="btn btn-light" style="border-radius: 0; display: <?php echo $previousDisplay; ?>;">previous page</button></a>
  <a href="http://54.188.51.218/panel/requests/?page=<?php echo $page+1; ?>"><button type="button" class="btn btn-primary" style="border-radius: 0; display: <?php echo $nextDisplay; ?>;">next page</button></a>

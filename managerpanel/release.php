...
<body>
  ...
  <div style="position: fixed; width: 50%; height: 100%; top: 6%; left: 20%">
    <h2 style="margin-top: 5%">tournament invitation / information authorization request</h2>
    ...
    <div class="list-group" style="width: 700">
      ...
      if($accessRecords->num_rows > 0) {
        while($row = $accessRecords->fetch_assoc()) { $recipient = $row["recipientName"]; ?>
          ...
          <span class="list-group-item" style="border: 0; text-align: left; padding: 0;">
            ...
            <br>
            <span><i>you agree to share your team's:</i></span><br>
            <?php if($row["d1"]) { echo "- Birth Certificate<br>"; }?>
            <?php if($row["d2"]) { echo "- Youth Player Registration Form<br>"; }?>
            <?php if($row["d3"]) { echo "- Player Card<br>"; }?>
            <?php if($row["d4"]) { echo "- Insurance Waiver<br>"; }?>
            <?php if($row["d5"]) { echo "- COVID-19 Waiver"; }?><br><br>
         </span>
      ...
      <div class="card" style="width: 600; border-radius:0; text-align: center">
        <div class="card-body">
          <strong>Error!</strong> Request not found. <a href="http://54.188.51.218/managerpanel">Retrun to Home</a>
        </div>
      </div>
     ...
     <?php  }
      ...
      <a href="http://54.188.51.218/managerpanel"><button type="button" class="btn btn-light btn-lg btn-block" style="border-radius: 0; width: 100%; font-weight: 600; margin-top: 10">cancel</button></a>
    </form>
    </span>
